...
export const Galeria = () => {
    ...
    async function cadastrarCard(e) {
        ...
        if (imagem && nomeImagem)
            try {
                //FormData é uma interface JavaScript que permite construir
                //um conjunto de pares chave/valor representando os dados de um formulário HTML.
                ...
                {cards.length > 0 ? (
                    ...
                    cards.map((e) => (
                        <Card
                            key={e.id}
                            tituloCard={e.nome}
                            imgCard={`https://localhost:7102/${e.caminho.replace("wwwroot/", "")}`}
                            funcaoEditar={() => editarCard(e.id)} 
                            funcaoExcluir={() => excluirCard(e.id, e.nome)} />
                    ))
